...
async fn h3_query_once(
    ...
    let mut send_req: h3::client::SendRequest<h3_quinn::OpenStreams, Bytes> = {
        ...
        if need_rebuild {
            ...
            let (mut driver, send_req) = h3::client::new(h3_quinn::Connection::new(quic_conn.clone()))
                .await
...
mod tests {
    ...
    fn h3_id_roundtrip() {
        ...
        let original_msg =
            Bytes::from_static(b"\xAB\xCD\x01\x00\x00\x01\x00\x00\x00\x00\x00\x00");
    ...
    fn h3_id_roundtrip_max_id() {
        let original_msg =
            Bytes::from_static(b"\xFF\xFF\x01\x00\x00\x01\x00\x00\x00\x00\x00\x00");
        let (rewritten, original_id) = set_h3_query_id_zero(original_msg.clone());
