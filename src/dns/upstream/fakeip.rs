...
mod tests {
    ...
    async fn fakeip_wrap_around_pointer_persistence() {
        ...
        //   （.2 被重用，覆盖 host0 的映射）
        let (cf1, rd1) = open_cache_file(f.path(), true, 7, false, 3600).unwrap();
        let store1 = FakeIpStore::new_with_cache(&cfg, Some(cf1.clone()), Some(rd1.clone())).unwrap();
        let _ = store1.reply(&make_fakeip_query("host0.example.com", 1)).unwrap();
        let _ = store1.reply(&make_fakeip_query("host1.example.com", 1)).unwrap();
        tokio::time::sleep(Duration::from_millis(150)).await;
        ...
        let resp = store2.reply(&make_fakeip_query("host2.example.com", 1)).unwrap();
        let ip_bytes: [u8; 4] = resp[resp.len() - 4..].try_into().unwrap();
        let ip = Ipv4Addr::from(ip_bytes);
        assert_eq!(
            ip,
    ...
    async fn fakeip_reset_clears_persistent_pointers() {
        ...
        let resp = store2.reply(&make_fakeip_query("fresh.example.com", 1)).unwrap();
        let ip_bytes: [u8; 4] = resp[resp.len() - 4..].try_into().unwrap();
        let ip = Ipv4Addr::from(ip_bytes);
        assert_eq!(
            ip,
