//! Local DNS 服务器（对齐 sing-box `dns/transport/local`）。
//!
//! 行为：
//! - 读取 `/etc/resolv.conf` 获取系统 DNS 服务器列表（5s 缓存 + mtime 热加载）
//! - 解析 nameserver / search / domain / options（ndots / attempts / timeout / rotate）
//! - A/AAAA 查询优先查 hosts 文件，未命中再走系统 DNS
//! - 系统 DNS 查询：UDP 优先，TC bit 或包过大时降级 TCP
//! - 支持 search domain + ndots 候选名展开（对齐 glibc/musl resolver 语义）
//!
//! 与 sing-box 的差异：
//! - 不实现 systemd-resolved D-Bus 集成（preferGo 隐式为 true）
//! - 不实现 Windows GetAdaptersAddresses（Windows 上仍读 `C:\Windows\System32\Drivers\etc\resolv.conf`? 实际 Windows 无此文件，会回退到 127.0.0.1:53）
//! - 不支持 detour（local 的查询直接走系统 socket，绕过 outbound 链）
...
impl LocalUpstream {
    ...
    async fn exchange_one(&self, cfg: &ResolvConfig, query: &[u8]) -> anyhow::Result<Bytes> {
        ...
        Err(last_err
            .unwrap_or_else(|| anyhow::anyhow!("local upstream: all attempts exhausted")))
    }
...
fn build_ip_response(query: &[u8], qtype: u16, addrs: &[IpAddr]) -> Bytes {
    ...
    let mut resp = Vec::with_capacity(query.len() + addrs.len() * (12 + rdata_len));
    ...
    resp.extend_from_slice(&query[12..]); // 回显 Question
    for addr in addrs {
...
";
    ...
    fn name_list_rooted_name() {
        let mut cfg = ResolvConfig::default();
        cfg.ndots = 1;
        cfg.search = vec!["example.com.".to_string()];
        // 已根化 → 直接返回单元素
    ...
    fn local_upstream_hosts_hit_skips_system_dns() {
        ...
        let upstream = LocalUpstream::with_paths(
            hosts_tmp.path().to_path_buf(),
            resolv_path.clone(),
        );
