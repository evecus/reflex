//! Hosts 文件 DNS 服务器（对齐 sing-box `dns/transport/hosts`）。
//!
//! 行为：
//! - 仅响应 A (1) / AAAA (28) 查询；其他类型直接返回 NXDOMAIN
//! - 查询优先级：predefined map → 按顺序遍历每个 hosts 文件
//! - 文件解析格式：`IP domain1 [domain2 ...]`，`#` 起注释作用（可出现在行中）
//! - 域名大小写不敏感（规范化为小写 + 末尾 `.`）
//! - 文件热重载：5 秒缓存窗口 + mtime/size 变化检测，失败时保留旧缓存
//!
//! 与 sing-box 的差异：
//! - 不支持 Windows 系统目录动态探测（固定回退到 `C:\Windows\System32\Drivers\etc\hosts`）
//! - 不实现 systemd-resolved 集成（那是 local 类型的职责）
