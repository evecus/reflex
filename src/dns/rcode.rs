//! Rcode 响应构造工具。
//!
//! 对齐 sing-box `dns.FixedResponseStatus`：
//! - flags = `QR=1, Opcode=0, AA=1, TC=0, RD=1, RA=1, Z=0, RCODE=rcode`
//!   即 byte2 = `0x85`，byte3 = `0x80 | rcode`
//! - **Question 段回显**：拷贝查询消息的 Question 段（按 QDCOUNT 解析）
//!   对齐 sing-box `Question: message.Question`
//!
//! 旧实现的 bug：
//! 1. flags 仅有 `0x80`（QR=1），缺 AA/RD/RA 位 → 客户端可能误判响应
//! 2. 完全没有 Question 段 → DNS 客户端可能拒绝响应（合规性要求 echo）
//! 3. QDCOUNT 始终为 0，与 Question 段不一致

use bytes::Bytes;

/// 计算 DNS 消息 Question 段的结束字节偏移。
///
/// Question 段从 `offset` 开始，格式为：
/// - QNAME: 一系列 label，每个为 `len(1) + data(len)`；终止符为 0 字节
/// - QTYPE: 2 字节
/// - QCLASS: 2 字节
///
/// 不支持 compression pointer（RFC 1035 规定 Question 段不允许有指针）。
/// 返回 None 表示解析失败（消息过短或格式错误）。
