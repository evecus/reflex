...
impl DnsResolver {
    ...
    pub fn from_config_full(
        ...
        let default = rule::resolve_server_ref(
            config.r#final.as_slice(),
            &upstreams,
            "dns.final",
        )?;
    ...
    async fn handle(&self, msg: Bytes, inbound_tag: &str) -> anyhow::Result<Bytes> {
        ...
        let (upstreams, disable_cache) = self
            ...
            .iter()
            .find(|r| {
                r.matches_normalized(inbound_tag, &qname_norm, qtype, &current_mode)
            })
                ...
                CacheResult::Stale(cached) => {
                    ...
                    tokio::spawn(async move {
                        ...
                        match cache2.try_lead_inflight(&transport_tag2, &qname2, qtype) {
                            ...
                            InflightResult::Leader => match race_upstreams(&upstreams2, &msg2).await {
                                Ok(resp) => {
