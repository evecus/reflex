    ...
    while let Some(a) = iter.next() {
        if a == "-o" {
            return Ok(Some(iter.next().cloned().ok_or_else(|| {
                anyhow::anyhow!("'-o' requires an argument")
            })?));
        }
...
fn cmd_ruleset(args: &[String]) -> anyhow::Result<()> {
    ...
    if args.len() < 2 {
        ...
        eprintln!("       reflex rs <input> [-o <output.rrs>] [-t adguard|yaml]  (rs = ruleset 简写)");
        process::exit(1);
    }
    let input = &args[1];
    let rest = &args[2..];
...
fn cmd_convert(args: &[String]) -> anyhow::Result<()> {
    ...
    let input = &args[1];
    let output = parse_output_flag(&args[2..])?.ok_or_else(|| {
        anyhow::anyhow!("missing required flag: -o <output.yaml|output.json>")
    })?;
