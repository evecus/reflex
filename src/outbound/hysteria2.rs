...
/// 旧实现用 `tokio::sync::Mutex<HashMap>`，每收到一个 datagram 都要
/// `.lock().await` 一次（异步互斥锁 + 调度点）。大流量 UDP（如视频/QUIC
...
/// 预估 header 长度（旧实现为测量 varint 长度分配一个临时 `BytesMut`，
/// 每个 UDP 包都要分配+释放，大流量下增加分配器压力）。
#[inline]
fn varint_len(i: u64) -> usize {
    if i <= 63 {
        1
    } else if i <= 16383 {
        2
...
/// 保持帧格式统一。
///
/// 性能优化（对齐 sing-quic `WritePacket`/`writePacket`）：
/// 1. header_overhead 用栈函数 `varint_len` 计算，不再分配临时 `BytesMut`。
/// 2. 单分片快路径（绝大多数 UDP 包 ≤ MTU）：直接构造一个 `BytesMut`，
///    避免 `chunks.collect()` 的 `Vec<&[u8]>` 分配与多片循环开销。
/// 3. 多分片路径用 `chunks()` 迭代器直接遍历，不 `collect` 成 `Vec`。
...
fn parse_udp_frag_header(buf: &Bytes) -> anyhow::Result<(Bytes, u8, u8, u32, u16)> {
    ...
    // 零拷贝切分：buf 是 quinn read_datagram 返回的 Bytes（引用计数底层
    // 分配），`slice` 仅偏移+长度，不复制数据。旧实现 `Bytes::copy_from_slice`
    // 每收一个分片复制一次 payload，大流量 UDP 下（每秒数千包）复制开销
    // 显著；且分片重组后多片 Bytes 拼接时又会再次复制。
...
mod tests {
    ...
    fn varint_len_matches_write_varint() {
        ...
        for val in [0u64, 1, 63, 64, 16383, 16384, 1_073_741_823, 1_073_741_824] {
            ...
            assert_eq!(
                varint_len(val),
                buf.len(),
                "varint_len({val}) = {} but write_varint wrote {}",
                varint_len(val),
                buf.len()
            );
        }
    ...
    #[test]
    fn parse_udp_frag_header_multifragment() {
        ...
        let frag0 = {
            ...
            b.put_u16(9); // packet_id
            b.put_u8(0); // frag_id
            b.put_u8(2); // frag_count
            write_varint(&mut b, addr.len() as u64);
            b.put_slice(addr.as_bytes());
        ...
        let (p0, fid0, fc0, sid0, pid0) = parse_udp_frag_header(&frag0).unwrap();
        let (p1, fid1, fc1, sid1, pid1) = parse_udp_frag_header(&frag1).unwrap();
        assert_eq!(&p0[..], b"abcde");
        assert_eq!(&p1[..], b"fghij");
        assert_eq!((fid0, fc0, sid0, pid0), (0, 2, 100, 9));
        assert_eq!((fid1, fc1, sid1, pid1), (1, 2, 100, 9));
    }

    #[test]
    fn parse_udp_frag_header_zero_copy_slice() {
        // 验证 parse_udp_frag_header 返回的 payload 是底层 Bytes 的零拷贝切片：
        // 修改原始 buf 的副本不影响已切出的 payload（slice 共享底层分配，
        // 但逻辑上独立）。这里验证 payload 内容正确即可。
        let addr = "x:1";
        let data = b"payload";
        let mut buf = build_udp_header(1, 1, 0, 1, addr);
        buf.put_slice(data);
        let frozen = buf.freeze();
        let (payload, _, _, _, _) = parse_udp_frag_header(&frozen).unwrap();
        assert_eq!(&payload[..], data);
        // payload 与 frozen 共享底层分配（零拷贝），但两者逻辑独立
