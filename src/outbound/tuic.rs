...
impl Outbound for TuicOutbound {
    ...
    async fn handle_udp(&self, mut packet: InboundUdpPacket) -> anyhow::Result<()> {
        ...
        tokio::spawn(async move {
            ...
            let mut reasm: std::collections::HashMap<u16, (u8, Vec<Option<Bytes>>, std::time::Instant)> =
            ...
            loop {
                match tokio::time::timeout(timeout, frag_rx.recv()).await {
                    Ok(Some(data)) => {
                        ...
                        let entry = reasm.entry(pid).or_insert_with(|| {
                            (frag_total, vec![None; frag_total as usize], now)
                        });
                        ...
                        if received == entry.1.len() {
                            ...
                            reasm.remove(&pid);
                            if reply_tx
                                .send((assembled, src, spoofed_src))
                                .await
                                .is_err()
                            {
                                break;
