...
/// 将一个 UDP 包按 `MAX_DATAGRAM_PAYLOAD` 分片并发送。
///
...
///
/// 性能优化（对齐 sing-quic `writePacket` 单次构造）：
/// 1. 单分片快路径（绝大多数 UDP 包 ≤ MTU）：直接构造一个 `BytesMut`
///    并 `send_datagram`，避免 `Vec<Bytes>` 与 `Vec<&[u8]>` 分配。
/// 2. 多分片路径用 `chunks()` 迭代器直接遍历，不 `collect` 成 `Vec`。
///
/// 旧实现 `build_udp_packets_fragmented` 返回 `Vec<Bytes>`，单分片也要
///    分配一个 Vec + 一个 Bytes，大流量下增加分配器压力。
...
///
/// 生产路径用 `send_udp_fragmented` 直接发送（避免 `Vec<Bytes>` 分配），
/// 本函数仅用于测试验证分片 wire 格式（frag_id/frag_total/ADDR 置空）。
/// 逻辑与 `send_udp_fragmented` 多分片路径一致，保持同步。
#[cfg(test)]
