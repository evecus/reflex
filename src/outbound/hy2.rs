...
fn build_quic_config(config: &Hysteria2OutboundConfig) -> anyhow::Result<Arc<quinn::ClientConfig>> {
    ...
    let cc_factory: Arc<dyn quinn::congestion::ControllerFactory + Send + Sync> = if tx_bps_local > 0 {
        // Brutal 近似：BBR + 大初始窗口
        let brutal_cwnd = (tx_bps_local * BRUTAL_APPROX_INITIAL_RTT.as_millis() as u64
            / 1000
            / 8)
            .max(1024);
        debug!(
