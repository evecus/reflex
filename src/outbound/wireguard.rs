...
fn build_udp_ip_packet(payload: &[u8], src: &SocketAddr, dst: &Target) -> anyhow::Result<Vec<u8>> {
    ...
    if is_v6 {
        let src_octets = match src.ip() { IpAddr::V6(ip) => ip.octets(), _ => unreachable!() };
        let dst_octets = match dst_addr.ip() { IpAddr::V6(ip) => ip.octets(), _ => unreachable!() };
        ...
        let src_ip = match src.ip() { IpAddr::V4(ip) => ip.octets(), _ => unreachable!() };
        let dst_ip = match dst_addr.ip() { IpAddr::V4(ip) => ip.octets(), _ => unreachable!() };
        let udp_len = 8 + payload.len();
        let ip_len = 20 + udp_len;
        let mut pkt = vec![0u8; ip_len];
        pkt[0] = 0x45; pkt[1] = 0;
        let total = ip_len as u16;
        pkt[2] = (total >> 8) as u8; pkt[3] = (total & 0xff) as u8;
        pkt[6] = 0x40; pkt[8] = 64; pkt[9] = 17;
        pkt[12..16].copy_from_slice(&src_ip);
        pkt[16..20].copy_from_slice(&dst_ip);
        let cksum = ip_checksum(&pkt[..20]);
        pkt[10] = (cksum >> 8) as u8; pkt[11] = (cksum & 0xff) as u8;
        pkt[20] = (src.port() >> 8) as u8; pkt[21] = (src.port() & 0xff) as u8;
        pkt[22] = (dst_addr.port() >> 8) as u8; pkt[23] = (dst_addr.port() & 0xff) as u8;
        pkt[24] = (udp_len >> 8) as u8; pkt[25] = (udp_len & 0xff) as u8;
...
fn parse_udp_ip_packet(pkt: &[u8]) -> anyhow::Result<(Vec<u8>, SocketAddr)> {
    if pkt.len() < 20 { return Err(anyhow!("IP packet too short")); }
    let version = (pkt[0] >> 4) & 0xf;
    match version {
        4 => {
            if pkt.len() < 28 { return Err(anyhow!("IPv4 packet too short")); }
            let ihl = (pkt[0] & 0xf) as usize * 4;
            if pkt[9] != 17 { return Err(anyhow!("only UDP proto supported")); }
        ...
        6 => {
            if pkt.len() < 48 { return Err(anyhow!("IPv6 packet too short")); }
            let nh = pkt[6];
            if nh == 0 {
                if pkt.len() < 50 { return Err(anyhow!("IPv6+ext packet too short")); }
                if pkt[48..50] != [0x00, 0x11] { return Err(anyhow!("IPv6: expected UDP frag")); }
            } else if nh != 17 {
                return Err(anyhow!("only UDP proto supported in IPv6"));
            }
            let header_end = 40usize;
            if pkt.len() < header_end + 8 { return Err(anyhow!("UDP header truncated")); }
...
fn ipv6_udp_checksum(src: &[u8; 16], dst: &[u8; 16], nxt: u8, udp_header: &[u8], payload: &[u8]) -> u16 {
    let mut sum: u32 = 0;
