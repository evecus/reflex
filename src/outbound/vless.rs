                ...
                async move {
                    ...
                    if let Some(VlessTransportConfig::Ws(ws_cfg)) = &transport {
                        let ws = crate::outbound::transport::websocket::connect(
                            &server, port, &sni, tls_cfg.as_ref(), ws_cfg, 0, None,
                        )
                        .await?;
                        ...
                        let stream = crate::outbound::transport::grpc::connect(
                            &server, port, &sni, tls_cfg.as_ref(), grpc_cfg, 0, None,
                        )
                        .await?;
                        let boxed: Box<dyn crate::outbound::common::smux::AsyncReadWrite> = Box::new(stream);
                        Ok(boxed)
                    } else if let Some(VlessTransportConfig::Xhttp(xhttp_cfg)) = &transport {
                        use std::collections::HashMap;
                        let stream = crate::outbound::transport::xhttp::connect(
                            &server, port, xhttp_cfg, tls_cfg.as_ref(), &HashMap::new(), 0, None,
                        )
                        .await?;
                        let boxed: Box<dyn crate::outbound::common::smux::AsyncReadWrite> = Box::new(stream);
                        Ok(boxed)
                        ...
                        if tls_cfg.as_ref().is_some_and(|t| t.enabled) {
                            ...
                            let boxed: Box<dyn crate::outbound::common::smux::AsyncReadWrite> = Box::new(tcp);
                            Ok(boxed)
                        }
                    }
                }
            };
            let mc = config.multiplex.as_ref().expect("checked Some above").clone();
