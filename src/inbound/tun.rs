...
mod platform {
    ...
    fn prefix_contains_v4(
        outer: (std::net::Ipv4Addr, u8),
        inner: (std::net::Ipv4Addr, u8),
    ) -> bool {
...
mod platform {
    ...
    pub fn setup(cfg: &TunInboundConfig, if_name: &str) -> anyhow::Result<()> {
        ...
        if cfg.strict_route {
            ...
            info!(
                "tun: strict_route DNS block rules added (Windows, UDP+TCP), reflex self exempted"
            );
...
mod tests {
    ...
    fn build_syn_v4_with_mss(mss: u16) -> Vec<u8> {
        ...
        // TCP header
        let tcp_off = 20;
        pkt[tcp_off + 0..tcp_off + 2].copy_from_slice(&0x1234u16.to_be_bytes()); // src port
