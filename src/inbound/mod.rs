...
pub struct SniffedStream {
    ...
    /// router 的单遍路由（route_with_state）在 dispatcher 热路径上会多次调用
    /// peer_addr()，缓存后只调用 1 次 getpeername syscall。失败时不缓存
    /// （连接可能已关闭，下次重试也无意义，但保持原语义：调用方拿到 Err）。
...
impl Drop for SniffedStream {
    fn drop(&mut self) {
        if self.rst_on_drop {
            ...
            #[allow(deprecated)]
            let _ = self.inner.set_linger(Some(std::time::Duration::ZERO));
        }
    }
}
