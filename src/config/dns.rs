            ...
            Repr::Many(v) => {
                if v.is_empty() {
                    return Err(serde::de::Error::custom(
                        "dns server list cannot be empty",
                    ));
                }
                Ok(Self(v))
            }
...
mod tests {
    ...
    fn parse_dns_string_or_array_fields() {
        ...
        let dns: DnsConfig = serde_json::from_value(v).unwrap();
        assert_eq!(dns.rules[0].query_type, vec![DnsQueryType::A, DnsQueryType::Aaaa]);
    }
    ...
    fn server_protocol_detection() {
        ...
        assert_eq!(
            make("h3://1.1.1.1/dns-query").protocol(),
            DnsProtocol::H3
        );
        assert_eq!(make("hosts://").protocol(), DnsProtocol::Hosts);
        assert_eq!(
            make("hosts:///etc/hosts").protocol(),
            DnsProtocol::Hosts
        );
    ...
    fn proxy_domain_resolver_short_form_array() {
        ...
        let dns: DnsConfig = serde_json::from_value(v).unwrap();
        let cfg = dns.proxy_domain_resolver.expect("should be Some");
        assert_eq!(cfg.server.as_slice(), &["local".to_string(), "remote".to_string()]);
    ...
    fn dns_rule_server_supports_array() {
        ...
        let dns: DnsConfig = serde_json::from_value(v).unwrap();
        assert_eq!(dns.rules[0].server.as_slice(), &["local".to_string(), "remote".to_string()]);
        assert_eq!(dns.r#final.as_slice(), &["local".to_string(), "remote".to_string()]);
    }
